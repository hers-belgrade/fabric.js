...
(function(global) {
    ...
    function loadImage(url, callback, context) {
        ...
        if (url) {
            ...
            if (image_pending[url]) {
                image_pending[url].push(function() {
                    report_done(context, callback, image_cahche[url]);
                });
                return;
            }
    ...
    function enlivenObjects(objects, namespace) {
        var ret = [];
    ...
    Matrix.getValidArr = function(m) {
        ...
        var v = m instanceof Matrix ? m.val : m instanceof Array && m.length == 6 ? m : undefined;
        if (!v) throw "Invalid argument for matrix transformation";
        return v;
    };
    Matrix.UnityMatrix = function() {
        return [ 1, 0, 0, 1, 0, 0 ];
    };
...
(function(global) {
    ...
    fabric.SHARED_ATTRIBUTES = [ "id", "inkscape:static", "transform", "fill", "fill-opacity", "fill-rule", "opacity", "display", "stroke", "stroke-dasharray", "stroke-linecap", "stroke-linejoin", "stroke-miterlimit", "stroke-opacity", "stroke-width" ];
    ...
    function gradientResolver(instance, attribute) {
        ...
        if (/^url\(/.test(attributeValue)) {
            ...
            if (fabric.gradientDefs[gradientId]) {
                instance.set(attribute, fabric.Gradient.fromElement(fabric.gradientDefs[gradientId], instance));
            }
        }
    }
...
(function(global) {
            ...
            return function(spriteobj) {
                ...
                return function(img) {
                    ...
                    for (var i in so) {
                        ...
                        var sg = so[i];
                        sprites[i] = new fabric.Sprite(img.getElement(), {
                            x: sg[0],
                            y: sg[1],
                            width: sg[2],
                            height: sg[3],
    ...
    function loadResources(resobj, cb, ctx) {
        function prepare_map(obj) {
            for (var i in obj) {
                if (!obj[i].name && !obj[i].root) obj[i] = {
                    name: obj[i],
                    root: resobj.root || ""
                };
            }
...
(function() {
    ...
    function _convertPercentUnitsToValues(object, options) {
        for (var prop in options) {
            ...
            if (prop === "x1" || prop === "x2") {} else if (prop === "y1" || prop === "y2") {}
        }
    }
...
(function() {
    var getPointer = fabric.util.getPointer, degreesToRadians = fabric.util.degreesToRadians, radiansToDegrees = fabric.util.radiansToDegrees, atan2 = Math.atan2, abs = Math.abs, min = Math.min, max = Math.max, STROKE_OFFSET = .5;
...
fabric.util.object.extend(fabric.StaticCanvas.prototype, {
    ...
    clone: function(callback) {
        var clone = this.cloneWithoutData();
        clone.loadFromJSON(JSON.stringify(this));
        return clone;
    },
...
(function(global) {
    "use strict";
    var fabric = global.fabric || (global.fabric = {}), extend = fabric.util.object.extend, toFixed = fabric.util.toFixed, capitalize = fabric.util.string.capitalize, matmult = fabric.util.multiplyTransformMatrices, degreesToRadians = fabric.util.degreesToRadians, Matrix = fabric.util.Matrix, supportsLineDash = fabric.StaticCanvas.supports("setLineDash");
    if (fabric.Object) {
        return;
    }
    fabric.Object = fabric.util.createClass({
        ...
        drawBorderRect: function(ctx) {
            ...
            if (this.oCoords.br.x - this.oCoords.tl.x >= 0 && this.oCoords.br.y - this.oCoords.tl.y >= 0) {
                ctx.strokeStyle = this.borderRectColor;
                ctx.strokeRect(this.oCoords.tl.x, this.oCoords.tl.y, this.oCoords.br.x - this.oCoords.tl.x, this.oCoords.br.y - this.oCoords.tl.y);
            } else {
...
(function(global) {
    ...
    fabric.Rect = fabric.util.createClass(fabric.Object, {
        ...
        toSVG: function() {
            var markup = this._createBaseSVGMarkup();
            markup.push("<rect ", 'x="0" y="0"', '" rx="', this.get("rx"), '" ry="', this.get("ry"), '" width="', this.width, '" height="', this.height, '" style="', this.getSvgStyles(), '" transform="', this.getSvgTransform(), '"/>');
            return markup.join("");
        },
        complexity: function() {
            return 1;
...
(function(global) {
    ...
    fabric.Polyline.fromObject = function(object) {
        return new fabric.Polyline(object.points, object, true);
    };
})(typeof exports !== "undefined" ? exports : this);
...
(function(global) {
    ...
    fabric.Path = fabric.util.createClass(fabric.Object, {
        ...
        toSVG: function() {
            ...
            markup.push('<g transform="', this.group ? "" : this.getSvgTransform(), '">', "<path ", 'd="', path, '" style="', this.getSvgStyles(), '" stroke-linecap="round" ', "/>", "</g>");
            return markup.join("");
        },
        complexity: function() {
        ...
        _improvedParsePath: function(path) {
            ...
            while (match) {
                ...
                result.push({
                    command: match[1],
                    params: match[2].trim().split(" ").map(function(v) {
...
(function(global) {
    ...
    fabric.Sprite = fabric.util.createClass(fabric.Image, {
        ...
        _render: function(ctx) {
            ctx.drawImage(this._element, this.x, this.y, this.width, this.height, 0, 0, this.width, this.height);
        },
        toObject: function(propertiesToInclude) {
            return extend(this.callSuper("toObject", propertiesToInclude), {
    ...
    fabric.StateableSprite = fabric.util.createClass(fabric.Sprite, {
        ...
        _stateGeometry: function(state) {
            return this._stategeometries[state] || {
                x: 0,
                y: 0,
                width: 0,
                height: 0
...
(function(global) {
    "use strict";
    var fabric = global.fabric || (global.fabric = {}), min = fabric.util.array.min, max = fabric.util.array.max, extend = fabric.util.object.extend, _toString = Object.prototype.toString, drawArc = fabric.util.drawArc, degreesToRadians = fabric.util.degreesToRadians, Matrix = fabric.util.Matrix, matmult = fabric.util.multiplyTransformMatrices;
    if (fabric.Use) {
...
(function(global) {
    ...
    stateProperties.push("fontFamily", "fontWeight", "fontSize", "path", "text", "textDecoration", "textAlign", "fontStyle", "lineHeight", "backgroundColor", "textBackgroundColor", "useNative");
    fabric.Text = fabric.util.createClass(fabric.Object, {
        ...
        _renderTextLinesBackground: function(ctx, textLines) {
            ...
            for (var i = 0, len = textLines.length; i < len; i++) {
                if (textLines[i] !== "") {
                    var lineWidth = this._getLineWidth(ctx, textLines[i]);
                    var lineLeftOffset = this._getLineLeftOffset(lineWidth);
                    ctx.fillRect(this._getLeftOffset() + lineLeftOffset, this._getTopOffset() + i * this._getFontSize() * this.lineHeight, lineWidth, this._getFontSize() * this.lineHeight);
                }
            }
            ctx.restore();
        },
        ...
        toSVG: function() {
            var textLines = this.text.split(/\r?\n/), lineTopOffset = this.useNative ? this._getFontSize() * this.lineHeight : -this._fontAscent - this._fontAscent / 5 * this.lineHeight, textLeftOffset = -(this.width / 2), textTopOffset = this.useNative ? this._getFontSize() - 1 : this.height / 2 - textLines.length * this._getFontSize() - this._totalLineHeight, textAndBg = this._getSVGTextAndBg(lineTopOffset, textLeftOffset, textLines), shadowSpans = this._getSVGShadows(lineTopOffset, textLines);
            textTopOffset += this._fontAscent ? this._fontAscent / 5 * this.lineHeight : 0;
            return [ '<g transform="', this.getSvgTransform(), '">', textAndBg.textBgRects.join(""), "<text ", this.fontFamily ? 'font-family="' + this.fontFamily.replace(/"/g, "'") + '" ' : "", this._getFontSize() ? 'font-size="' + this._getFontSize() + '" ' : "", this.fontStyle ? 'font-style="' + this.fontStyle + '" ' : "", this.fontWeight ? 'font-weight="' + this.fontWeight + '" ' : "", this.textDecoration ? 'text-decoration="' + this.textDecoration + '" ' : "", 'style="', this.getSvgStyles(), '" ', 'transform="translate(', toFixed(textLeftOffset, 2), " ", toFixed(textTopOffset, 2), ')">', shadowSpans.join(""), textAndBg.textSpans.join(""), "</text>", "</g>" ].join("");
        },
    ...
    fabric.Text.ATTRIBUTE_NAMES = fabric.SHARED_ATTRIBUTES.concat("x y font-family font-style font-weight font-size text-decoration text-align".split(" "));
...
(function(global) {
    ...
    fabric.Tspan = fabric.util.createClass(fabric.Text, {});
...
(function(global) {
    ...
    fabric.Hoverable = function(svgelem, config) {
        ...
        svgelem.on("object:over", function() {
            overcb.call(ctx, svgelem);
        });
        svgelem.on("object:out", function() {
            outcb.call(ctx, svgelem);
        });
        return svgelem;
    };
})(typeof exports !== "undefined" ? exports : this);
...
(function(global) {
    ...
    fabric.Clickable = function(svgelem, config) {
        ...
        svgelem.on("mouse:down", function() {
            downcb.call(ctx, svgelem);
        });
        svgelem.on("mouse:up", function() {
            clickcb.call(ctx, svgelem);
        });
        return svgelem;
    };
})(typeof exports !== "undefined" ? exports : this);
...
(function(global) {
    ...
    fabric.TextButton = function(svgelem, config) {
        ...
        var ra = function() {
            rainvoker.invokeOnCanvas("renderAll");
        };
